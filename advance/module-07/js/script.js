...
//создание элементов списка
      let topRowItems = this.layouts[this.langs[this.currentLang]].topRow.map(n => n = '<li></li>').reduce((acc, next) => acc + next);
      let middleRowItems = this.layouts[this.langs[this.currentLang]].middleRow.map(n => n = '<li></li>').reduce((acc, next) => acc + next);
      let bottomRowItems = this.layouts[this.langs[this.currentLang]].bottomRow.map(n => n = '<li></li>').reduce((acc, next) => acc + next);
...
//запись букв текущего языка в кнопки клавиатуры
      return arrTop.map((el, i) => el.textContent = this.layouts[this.langs[this.currentLang]].topRow[i])+
             arrMiddle.map((el, i) => el.textContent = this.layouts[this.langs[this.currentLang]].middleRow[i])+
             arrBottom.map((el, i) => el.textContent = this.layouts[this.langs[this.currentLang]].bottomRow[i]);
    },
